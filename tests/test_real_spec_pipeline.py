...
def test_kawasaki_section_detail_to_bundle_generation_persists_across_sqlite_app_instances(tmp_path: Path) -> None:
    db_path = tmp_path / "kawasaki-section-detail-to-bundle-sqlite.db"
    client_a = make_sqlite_client(db_path)

    section_response = client_a.get("/pipelines/markdown-specs/kawasaki-global-req/sections/sec_001")
    assert section_response.status_code == 200
    section = section_response.json()
    assert section["title"] == "テストスペック"
    assert "49245-1528を満足すること" in section["text"]

    bundle_response = client_a.post(
        f"/pipelines/markdown-specs/kawasaki-global-req/sections/{section['section_key']}/generate-requirement-bundle",
        json={
            "prompt_version": "deterministic-note-v1",
            "model_version": "rule-based-generator-v1",
            "variant_scope": "base",
    ...
    submit_response = client_a.post(
        f"/requirements/{requirement_id}/submit-review",
        json={"reviewer_id": "reviewer-a", "review_note": "ready from kawasaki section detail sqlite path"},
    )
    assert submit_response.status_code == 200
    ...
    approve_response = client_a.post(
        ...
        json={
            ...
            "decision": "APPROVED",
            "reviewer_id": "reviewer-a",
            "review_note": "Approved from kawasaki section detail sqlite path.",
        },
    )
    assert approve_response.status_code == 201
    assert approve_response.json()["artifact"]["audit_rationale_id"] == audit_id
    ...
    section_reloaded = client_b.get("/pipelines/markdown-specs/kawasaki-global-req/sections/sec_001")
    assert section_reloaded.status_code == 200
    assert section_reloaded.json()["id"] == section["id"]
