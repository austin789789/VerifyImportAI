...
3. **全資料留存 (Auditability)**: 儲存所有 LLM 原始輸入/輸出、Prompt 版本與人工修正紀錄。
4. **模組化變體**: 支援 Base + Overlay 模式管理多機種差異。
5. **知識閉環 (Knowledge Loop)**: 系統從人工核准且經過資深架構師精選的資料中自動學習，建立具備時效性 (3 年) 的「白銀資料集」。
...
3. **Signal Integration**: 當底層訊號 (DBC/ARXML) 變更時，觸發 **Signal-Sync Agent** 自動修正需求數值。
...
5. **品質預警機制 (Quality Hotspot Map)**: 視覺化呈現低置信度或高退回率的規格章節。
6. **預算控管 (Budget Cap)**: 以章節為單位預估 Token 消耗。
...
3. **AI 輔助檢漏**: 系統自動提示可能遺漏的需求、測試需求。
4. **AI 輔助撰寫測試案例**: 系統依據Graph自動產出測試案例。
